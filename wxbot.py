...
class WXBot:
    ...
    def proc_at_info_zz(msg):
        ...
        if (atcount > 0) :
            ...
            pm = re.search(pattern, msg)
            
            str_msg_all = msg
            str_msg = msg
            infos = []
            for i in range(atcount) :
                infos.append({'type': 'at', 'value': pm.group(i + 1).replace('@', '')})
                str_msg = str_msg.replace(pm.group(i + 1), '')
                
            infos.append({'type': 'str', 'value': str_msg})
        else:
            infos.append({'type': 'str', 'value': msg})
            str_msg_all = msg
            str_msg = msg
        return str_msg_all.strip(), str_msg.strip(), infos
        ...
        if mtype == 1:
            if content.find('http://weixin.qq.com/cgi-bin/redirectforward?args=') != -1:
                ...
                if msg_type_id == 3 or (msg_type_id == 1 and msg['ToUserName'][:2] == '@@'):  # Group text message
                    ...
                    msg_infos = self.proc_at_info_zz(content)
